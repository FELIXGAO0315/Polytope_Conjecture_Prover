    ...
    def build(
        ...
        if target_f2 <= 50:
            G = _astar_chop(
                ...
                timeout=timeout * 0.45,   # 45% of budget
            )
            if G is not None:
                return G, "astar_from_dodecahedron"

        # Strategy 4: A* from tetrahedron (fallback for small / exotic p-vectors)
        if target_f2 <= 30:
            G = _astar_chop(
                nx.complete_graph(4), nonzero,
                timeout=timeout * 0.25,  # 25% of budget
            )
            if G is not None:
                return G, "astar_from_tetrahedron"

        # Strategy 5: A* from kmax-gon prism.
        ...
        if kmax >= 7 and target_f2 <= 60:
            G = _astar_chop(
                ...
                timeout=timeout * 0.25,  # remaining 25%
            )
            if G is not None:
