...
from agent.prompts.lean_generation import (
    FIX_LOOP_POLIB_REF,
...
from agent.tools.blueprint import Blueprint, BlueprintDecomposer, BlueprintNode
from agent.tools.formalization_logger import FormalizationLogger
from agent.tools.goal_lock import GoalExtractor, GoalLock, GoalValidator, LockedGoal
from agent.tools.latex_parser import LatexParser, ParsedTheorem
from agent.tools.lean_compiler import LeanCompiler
from agent.tools.polib_manager import DepGraphManager, PolibManager, SessionState, StoreManager
from agent.tools.quality_checker import QualityChecker
from agent.tools.search import CombinedHintGenerator, GitHubLean4Search, MathlibSearch, PolibSearch
from agent.tools.prompt_optimizer import extract_signature, filter_dep_imports_to_direct
from agent.tools.output_search import OutputSearch
from agent.tools.llm_hint_generator import LLMHintGenerator, FailureRecord
...
class FormalizerAgent:
    ...
    def __init__(self, config: Config):
        ...
        from agent.tools.search import LLMProofReasoningHintGenerator
        ...
        try:
            from agent.tools.loogle_validator import LoogleValidator
    ...
    def _ensure_polib_lean(polib_path: Path) -> None:
        ...
        from agent.tools.polib_manager import _SECTION_MARKER
    ...
    def _write_output(self, lean_code: str, module_name: str, success: bool) -> None:
        out_dir = self._output_root / "Output"
        out_dir.mkdir(parents=True, exist_ok=True)
        ts = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        status_line = "-- compile: OK" if success else "-- compile: FAILED"
        # Strip any stale output headers from previous rounds before writing
        lines = lean_code.splitlines()
        while lines and (lines[0].startswith("-- Output/") or lines[0].startswith("-- generated_at:") or lines[0].startswith("-- compile:")):
            lines.pop(0)
        clean_code = "\n".join(lines)
        with self._output_lock:
            (out_dir / f"{module_name}.lean").write_text(
                f"-- Output/{module_name}.lean\n"
                f"-- generated_at: {ts}\n"
                f"{status_line}\n"
                + clean_code,
                encoding="utf-8",
    ...
    def _blueprint_cache_key(self, parsed: ParsedTheorem, goal_lock: GoalLock) -> str:
        ...
        from agent.prompts.lean_generation import SHARED_MODULE_CONTENT
    ...
    def _try_sorry_save(
        ...
        """
        from agent.tools.lean_compiler import CompileResult as _CR
        _dummy = _CR(
    ...
    def formalize(
        ...
        from agent.tools.polib_validator import PolibValidator
        ...
        if tex_path:
            try:
                cp, code_missing, asm_sorry_count = self._write_complete_proof_file(
                    tex_path, parsed.name, nodes_proved, nodes_partial, nodes_failed, sorry_total
                )
                if code_missing:
                    self._log(verbose, f"[complete proof] WARNING: code missing for {code_missing} — moved to failed")
                    for nid in code_missing:
                        if nid in nodes_proved:
                            nodes_proved.remove(nid)
                        elif nid in nodes_partial:
                            nodes_partial.remove(nid)
                        if nid not in nodes_failed:
                            nodes_failed.append(nid)
                if asm_sorry_count > sorry_total:
                    self._log(verbose, f"[complete proof] WARNING: {asm_sorry_count} sorry(s) detected in assembled output (stale cache)")
                    sorry_total = asm_sorry_count
                self._log(verbose, f"[complete proof] {cp}")
            except Exception as exc:
                self._log(verbose, f"[complete proof] write failed: {exc}")
