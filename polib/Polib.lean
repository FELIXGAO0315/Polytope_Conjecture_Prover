...
-- === C193_CaseZeroS (proved) ===
-- quality_score: 1.000 | sorry_count: 0 | saved_at: 2026-07-05T12:05:49.922307+00:00
private lemma C193_CaseZeroS (maps : SimplyCon3ConnectedMap 0) (hM : IsMap maps) (h1 : maps.p_i 3 = 0) (h2 : ∑ k ∈ Finset.Ico 3 (maps.m + 1), maps.p_i k ≥ 17) (hS : ∑ k ∈ Finset.Ico 7 (maps.m + 1), maps.p_i k = 0) : (maps.p_i 6 : ℤ) ≥ 4 := by
  have hEuler := Juc_EulerFormula maps hM
  have hzero_k : ∀ k ∈ Finset.Ico 7 (maps.m + 1), maps.p_i k = 0 := by
    intro k hk
    exact (Finset.sum_eq_zero_iff.mp hS) k hk
  have hsum_weight : ∑ k ∈ Finset.Ico 7 (maps.m + 1), ((k : ℤ) - 6) * (maps.p_i k : ℤ) = 0 := by
    apply Finset.sum_eq_zero
    intro k hk
    simp [hzero_k k hk]
  have h1z : (maps.p_i 3 : ℤ) = 0 := by exact_mod_cast h1
  have hEq : 2 * (maps.p_i 4 : ℤ) + (maps.p_i 5 : ℤ) = 12 := by linarith
  have h2z : ∑ k ∈ Finset.Ico 3 (maps.m + 1), (maps.p_i k : ℤ) ≥ 17 := by exact_mod_cast h2
  have hSz : ∑ k ∈ Finset.Ico 7 (maps.m + 1), (maps.p_i k : ℤ) = 0 := by exact_mod_cast hS
  have hprg := p_range maps hM
  have hIco : ∑ k ∈ (Finset.Ico 3 7 : Finset ℕ), (maps.p_i k : ℤ) ≥ 17 := by
    by_cases hm : 7 ≤ maps.m + 1
    · have hsplit := Finset.sum_Ico_consecutive (fun k => (maps.p_i k : ℤ)) (by norm_num : (3:ℕ) ≤ 7) hm
      linarith
    · push_neg at hm
      have hsub : Finset.Ico 3 (maps.m + 1) ⊆ Finset.Ico 3 7 :=
        Finset.Ico_subset_Ico_right (by omega)
      have hext : ∑ k ∈ Finset.Ico 3 (maps.m + 1), (maps.p_i k : ℤ) =
                  ∑ k ∈ (Finset.Ico 3 7 : Finset ℕ), (maps.p_i k : ℤ) := by
        apply Finset.sum_subset hsub
        intro k hk hknot
        simp [Finset.mem_Ico] at hk hknot
        have hkgt : maps.m < k := by omega
        exact_mod_cast (hprg k hkgt)
      linarith
  have hExp : ∑ k ∈ (Finset.Ico 3 7 : Finset ℕ), (maps.p_i k : ℤ) =
              (maps.p_i 3 : ℤ) + (maps.p_i 4 : ℤ) + (maps.p_i 5 : ℤ) + (maps.p_i 6 : ℤ) := by
    rw [show (Finset.Ico 3 7 : Finset ℕ) = {3, 4, 5, 6} from by decide]
    simp
    ring
  have hp4 : (0 : ℤ) ≤ (maps.p_i 4 : ℤ) := Int.ofNat_nonneg _
  linarith

-- === C215_DSRelation (proved) ===
-- quality_score: 1.000 | sorry_count: 0 | saved_at: 2026-07-05T16:31:24.033922+00:00
...
-- quality_score: 1.000 | sorry_count: 0 | saved_at: 2026-07-05T16:33:49.812146+00:00
open Finset
...
-- quality_score: 1.000 | sorry_count: 0 | saved_at: 2026-07-05T16:36:02.322468+00:00
...
-- quality_score: 1.000 | sorry_count: 0 | saved_at: 2026-07-05T16:38:04.878150+00:00
...
-- quality_score: 1.000 | sorry_count: 0 | saved_at: 2026-07-05T16:45:59.010988+00:00
open Finset
...
-- === C215 (proved) ===
-- quality_score: 1.000 | sorry_count: 0 | saved_at: 2026-07-05T16:50:28.212355+00:00
